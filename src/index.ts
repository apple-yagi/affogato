...
const run = async (
  ...
  const allTestsPackages = getRunAllTestsPackages(runAllTestsPackages, runAllTestsConfig);
...
(async () => {
  try {
    ...
    const results = await run(tsconfig, token, runAllTestsPackages, runAllTestsConfig);
