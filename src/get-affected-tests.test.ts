...
import {
  ...
  getFilesUsingPackages,
} from "./get-affected-tests.js";
...
test("getFilesUsingPackages: finds files importing react", () => {
  ...
  expect(result.some((file: string) => file.includes("react-component"))).toBe(
    true
  );
});
...
test("getAffectedTestFiles: includes tests for files using changed packages", () => {
  ...
  expect(
    result.affectedTests.some((file: string) =>
      file.includes("react-component.test")
    )
  ).toBe(true);
});
...
test("getAffectedTestFiles: combines changed files and changed packages", () => {
  ...
  expect(
    result.affectedTests.some((file: string) => file.includes("foo.test"))
  ).toBe(true);
  expect(
    result.affectedTests.some((file: string) =>
      file.includes("react-component.test")
...
test("getAffectedTestFiles: includes transitive dependencies from package changes", () => {
  ...
  expect(
    result.affectedTests.some((file: string) =>
      file.includes("use-react-components.test")
    )
  ).toBe(true);
});
